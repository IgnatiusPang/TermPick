...
distance_window_threshold <- 5
num_conditions_threshold <- 1
...
project_output_dir <- file.path( results_dir, "Sites" )
...
if( use_log == TRUE) {
  selected_parameter_file <- file.path( selected_params_dir, 
                          "condition_vs_position_clusters_win8_sd4_cpm1_gw5.tab")
...
dist5_cpm1_counts <- counts_table_clean %>%
  ...
  ungroup() %>%
  dplyr::filter( num_condition_identified >= num_conditions_threshold )# %>%
 # dplyr::filter( cpm_cutoff == replicate_cpm_cutoff &
   #                distance_window == distance_window_threshold )

if (nrow(dist5_cpm1_counts) > 0) {
